...
class Admin_Page
{
    ...
    public function __construct($framework)
    {
        ...
        // ✨ ADD THIS LINE to load the WP Media Uploader scripts
        wp_enqueue_media();
    }
            ...
            <div class="asf-container mw-100"  style="margin-bottom: -65px !important;" :data-bs-theme="theme">
                ...
                <header class="asf-header d-flex align-items-center px-4 justify-content-between bg-light-subtle border-bottom py-3">
                    <div class="d-flex align-items-center">
                        <div>
                            <button class="btn btn-sm btn-outline-secondary d-lg-none me-3" type="button" data-bs-toggle="collapse" data-bs-target="#asf-sidebar" aria-expanded="true" aria-controls="asf-sidebar"><i class="fa-solid fa-bars"></i></button>
                        </div>
                        <div class="">
                            <h1 class="h6 mb-0 text-center fw-bold"><?php echo esc_html($plugin_name); ?></h1>
                        </div>
                    </div>
                    ...
                    <div class="d-flex align-items-center">
                        ...
                        <div class="mx-2 animate-scale">
                            <button @click="toggleTheme()" class="bs-dark-mode-toggle btn btn-icon fs-6 btn-icon rounded-circle" role="button" data-bs-toggle="tooltip" data-bs-placement="bottom" data-bs-title="Toggle dark mode" aria-label="Toggle dark mode">
                        ...
                        <button class="btn btn-primary" @click="saveSettings()" :disabled="isLoading">
                            ...
                            <span x-text="isLoading ? strings.saving : '<?php _e('Save Changes', 'ayecode-settings-framework'); ?>'" class="d-none d-md-block"></span>
                ...
                <div class="asf-settings-page d-flex" style="min-height: calc(100vh - 107px);">
                            ...
                            <nav class="nav flex-column py-2">
                                <template x-for="section in sections" :key="section.id">
                                    <div class="sidebar-nav-section">
                                        <a href="#" class="nav-link text-dark-subtle  d-flex align-items-center w-100 p-3"
                    ...
                    <div class="main-content w-100 d-flex flex-column justify-content-between position-relative bg-secondary-subtle">
                                    ...
                                    <div>
                                        <template x-if="currentSectionData.subsections">
                                            <template x-if="currentSubsectionData">
                                                <div>
                                                    ...
                                                    <hr class="mt-4 mb-0">
                                                    <template x-for="(field, index) in currentSubsectionData.fields" :key="index">
                                                        <div class="py-4" x-html="renderField(field)"></div>
                                                    </template>
                                                </div>
                                            </template>
                                        </template>
                                        <template x-if="!currentSectionData.subsections">
                                            <div>
                                                ...
                                                <hr class="mt-4 mb-0">
                                                <template x-for="(field, index) in currentSectionData.fields" :key="index">
                                                    <div class="py-4" x-html="renderField(field)"></div>
                                                </template>
                                            </div>
                                        </template>
                        ...
                        <div class="asf-save-bar bg-light-subtle border-top border-start p-3 position-absolute w-100 bottom-0 text-body" x-show="hasUnsavedChanges" x-cloak x-transition>
                        ...
                        <div class="position-fixed top-0 start-0 w-100 h-100" style="background-color: rgba(0, 0, 0, 0.5); z-index: 99999;" x-show="isLoading" x-cloak>
                ...
                <div class="modal fade" id="asf-search-modal" tabindex="-1" aria-labelledby="searchModalLabel" aria-hidden="true">
                    <div class="modal-dialog modal-dialog-scrollable modal-lg">
                        <div class="modal-content">
                            ...
                            <div class="modal-body">
                                ...
                                <div x-show="searchQuery && groupedSearchResults.length === 0" class="text-center text-muted p-5" x-cloak>
                                    <p><?php _e('No results found for', 'ayecode-settings-framework'); ?> <strong x-text="searchQuery"></strong></p>
                                </div>
                                                ...
                                                <template x-for="result in group.results" :key="result.field.id">
                                                    <li class="list-group-item list-group-item-action border-0 rounded py-1">
                                                        <a href="#" @click.prevent="goToSearchResult(result)" class="text-decoration-none text-dark-subtle  d-block p-2">
                                                            ...
                                                            <div class="small text-muted" x-show="result.field.description" x-text="result.field.description"></div>
                                                        </a>
                                                    </li>
                                                </template>
                                            </ul>
