(function(){"use strict";function T(e){e.config?.sections&&(e.sections=e.config.sections.map(t=>({...t})))}function j(e){e.allFields=[];const t=(i,s,n=null)=>{Array.isArray(i)&&i.forEach(o=>{o&&(o.type==="group"&&o.fields?t(o.fields,s,n):o.id&&o.searchable!==!1&&e.allFields.push({type:"field",field:o,sectionId:s.id,sectionName:s.name,subsectionId:n?n.id:null,subsectionName:n?n.name:null,icon:s.icon}))})};e.sections.forEach(i=>{e.allFields.push({type:"section",id:i.id,name:i.name,icon:i.icon,keywords:i.keywords||[]}),t(i.fields,i),i.subsections?.forEach(s=>{e.allFields.push({type:"subsection",id:s.id,name:s.name,icon:i.icon,sectionId:i.id,sectionName:i.name,keywords:s.keywords||[]}),t(s.fields,i,s)})})}function v(e){if(Array.isArray(e)){if(e.length===0)return;const t=e.map(v).filter(i=>i!==void 0);return t.length>0?t:void 0}if(typeof e=="object"&&e!==null){const t=Object.entries(e).reduce((i,[s,n])=>{const o=v(n);return o!==void 0&&(i[s]=o),i},{});return Object.keys(t).length===0?void 0:t}return e}function k(e){const t=e.activePageConfig;if(!t||["form_builder","custom_page","action_page","import_page","tool_page"].includes(t.type))return!1;const s=t.fields,n=Array.isArray(s)?s:typeof s=="object"&&s!==null?Object.values(s):[];if(n.length===0)return!1;const o=a=>{const r=["title","group","alert","action_button"];return a.some(c=>{if(c.type==="group"&&c.fields){const l=Array.isArray(c.fields)?c.fields:Object.values(c.fields);return o(l)}return!r.includes(c.type)})};return o(n)}function I(e){const t=e.activePageConfig;if(!t)return!1;if(t.type==="form_builder"){const i=t.id,s=e.settings[i]||[],n=e.originalSettings[i]||[],o=JSON.parse(JSON.stringify(s)).map(l=>(delete l.fields,l)),a=JSON.parse(JSON.stringify(n)).map(l=>(delete l.fields,l)),r=v(o),c=v(a);return JSON.stringify(r)!==JSON.stringify(c)}if(k(e)){const i=Array.isArray(t.fields)?t.fields:Object.values(t.fields||{}),s=n=>{for(const o of n)if(o.type==="group"&&o.fields){const a=Array.isArray(o.fields)?o.fields:Object.values(o.fields);if(s(a))return!0}else if(o.id){const a=e.settings[o.id],r=e.originalSettings[o.id];if(JSON.stringify(a)!==JSON.stringify(r))return!0}return!1};return s(i)}return!1}function D(e){if(!e.activePageConfig||!e.activePageConfig.fields)return!0;document.querySelectorAll(".asf-field-error").forEach(i=>i.classList.remove("asf-field-error"));const t=Array.isArray(e.activePageConfig.fields)?e.activePageConfig.fields:Object.values(e.activePageConfig.fields);for(const i of t)if(i.extra_attributes?.required){const s=e.settings[i.id];if(s===""||s===null||s===void 0||Array.isArray(s)&&s.length===0){e.showNotification(`Error: The "${i.label||i.id}" field is required.`,"error");const n=document.getElementById(i.id);if(n){const o=n.closest(".row");o&&(o.classList.add("asf-field-error"),o.scrollIntoView({behavior:"smooth",block:"center"}),setTimeout(()=>o.classList.remove("asf-field-error"),3500))}return!1}}return!0}async function V(e){if(D(e)){e.isLoading=!0;try{const i=await(await fetch(window.ayecodeSettingsFramework.ajax_url,{method:"POST",headers:{"Content-Type":"application/x-www-form-urlencoded"},body:new URLSearchParams({action:window.ayecodeSettingsFramework.action,nonce:window.ayecodeSettingsFramework.nonce,settings:JSON.stringify(e.settings)})})).json();i.success?(e.settings=i.data.settings,e.originalSettings=JSON.parse(JSON.stringify(e.settings)),e.originalImagePreviews=JSON.parse(JSON.stringify(e.imagePreviews)),e.showNotification(i.data?.message||e.strings.saved,"success")):e.showNotification(i.data?.message||e.strings.error,"error")}catch(t){console.error("Save error:",t),e.showNotification(e.strings.error,"error")}finally{e.isLoading=!1}}}async function J(e){e.isLoading=!0;const t=e.activePageConfig.id,i=e.config.sections.find(a=>a.id===t),s=e.editingField?e.editingField._uid:null,n=JSON.parse(JSON.stringify(e.settings[t]));n.forEach(a=>delete a.fields);const o={[t]:n};try{const r=await(await fetch(window.ayecodeSettingsFramework.ajax_url,{method:"POST",headers:{"Content-Type":"application/x-www-form-urlencoded"},body:new URLSearchParams({action:window.ayecodeSettingsFramework.action,nonce:window.ayecodeSettingsFramework.nonce,settings:JSON.stringify(o),is_partial_save:!0})})).json();if(r.success){const c=r.data.settings[t],l=i.templates.flatMap(g=>g.options),d=c.map(g=>{const m=l.find(h=>h.id===g.template_id);return m&&(g.fields=m.fields),g});if(e.settings[t]=d,e.originalSettings[t]=JSON.parse(JSON.stringify(d)),s&&s.toString().startsWith("new_")){const g=d.find(m=>m.template_id===e.editingField.template_id&&!e.originalSettings[t].some(h=>h._uid===m._uid));g&&(e.editingField=g)}e.leftColumnView="field_list",e.editingField=window.__ASF_NULL_FIELD,e.showNotification(r.data?.message||"Form saved!","success")}else e.showNotification(r.data?.message||e.strings.error,"error")}catch(a){console.error("Save error:",a),e.showNotification(e.strings.error,"error")}finally{e.isLoading=!1}}function M(e){confirm(e.strings.confirm_discard)&&(e.settings=JSON.parse(JSON.stringify(e.originalSettings)),e.imagePreviews=JSON.parse(JSON.stringify(e.originalImagePreviews)))}function R(e,t="settings"){let i="";if(window.asfFieldRenderer){const s="render"+e.type.charAt(0).toUpperCase()+e.type.slice(1)+"Field";typeof window.asfFieldRenderer[s]=="function"?i=window.asfFieldRenderer[s](e):typeof window.asfFieldRenderer.renderField=="function"?i=window.asfFieldRenderer.renderField(e):i=`<div class="alert alert-warning">Field renderer for type "${e.type}" not found.</div>`}else i=`<div class="alert alert-warning">Field renderer for type "${e.type}" not found.</div>`;if(t!=="settings"){const s=new RegExp('(x-model|:checked|@click|x-show)="(settings|\\s*settings)\\.',"g");i=i.replace(s,`$1="${t}.`)}return i}function y(e){return e.sections.find(t=>t.id===e.currentSection)}function x(e){const t=y(e);return t?.subsections?t.subsections.find(i=>i.id===e.currentSubsection):null}function U(e){return x(e)||y(e)||null}function _(e){if(e.sections.length>0){e.currentSection=e.sections[0].id;const t=y(e);t?.subsections?.length>0&&(e.currentSubsection=t.subsections[0].id),t?.type==="custom_page"&&t.ajax_content&&e.loadCustomPageContent(e.currentSection)}b(e)}function L(e){const t=window.location.hash.substring(1);if(!t){_(e);return}const i=new URLSearchParams(t),s=i.get("section"),n=i.get("subsection"),o=i.get("field"),a=e.sections.find(r=>r.id===s);a?(e.currentSection=s,a?.type==="custom_page"&&a.ajax_content&&e.loadCustomPageContent(s),n&&a.subsections?.some(r=>r.id===n)?e.currentSubsection=n:e.currentSubsection=a.subsections?.length?a.subsections[0].id:""):_(e),o&&e.highlightField(o)}function b(e,t=null){const i=new URLSearchParams;e.currentSection&&i.set("section",e.currentSection),e.currentSubsection&&i.set("subsection",e.currentSubsection),t&&i.set("field",t);const s=i.toString();history.replaceState(null,"",s?`#${s}`:window.location.pathname+window.location.search)}function H(e,t,i=""){e.changeView(()=>{e.currentSection=t;const s=e.sections.find(n=>n.id===t);e.currentSubsection=i||(s?.subsections?.length?s.subsections[0].id:""),e.searchModal?.hide?.(),b(e),s?.type==="custom_page"&&s.ajax_content&&e.loadCustomPageContent(t)})}function q(e,t){e.changeView(()=>{e.currentSection=t,e.sidebarOpen=!1;const i=e.sections.find(s=>s.id===t);e.currentSubsection=i?.subsections?.length?i.subsections[0].id:"",b(e),i?.type==="custom_page"&&i.ajax_content&&e.loadCustomPageContent(t)})}function B(e,t){e.currentSubsection!==t&&e.changeView(()=>{e.currentSubsection=t,b(e)})}function W(e){e.searchModalEl=document.getElementById("asf-search-modal"),e.searchModalEl&&(e.searchModal=new bootstrap.Modal(e.searchModalEl),e.searchModalEl.addEventListener("shown.bs.modal",()=>document.getElementById("asf-search-input")?.focus()),e.searchModalEl.addEventListener("hidden.bs.modal",()=>e.searchQuery=""))}function G(e){if(!e.searchQuery.trim())return[];const t=e.searchQuery.toLowerCase().trim(),n=e.allFields.filter(a=>a.type==="field").filter(a=>{const r=a.field;return[r.label,r.description,a.sectionName,a.subsectionName,...r.keywords||[]].join(" ").toLowerCase().includes(t)}).reduce((a,r)=>{const c=r.subsectionName||r.sectionName,l=r.subsectionName?`${r.sectionName} &raquo; ${r.subsectionName}`:r.sectionName;return a[c]||(a[c]={groupTitle:l,sectionIcon:r.sectionIcon,results:[],sectionId:r.sectionId,subsectionId:r.subsectionId}),a[c].results.push(r),a},{}),o=(e.customSearchLinks||[]).filter(a=>[a.title,a.description,...a.keywords||[]].join(" ").toLowerCase().includes(t));return o.length&&(n.helpful_links={groupTitle:"Helpful Links",sectionIcon:"fas fa-fw fa-external-link-alt",results:o,isCustomGroup:!0}),Object.values(n)}function z(e,t){e.changeView(()=>{e.currentSection=t.sectionId,e.currentSubsection=t.subsectionId||"",e.searchModal.hide(),e.updateUrlHash(t.field.id),e.$nextTick(()=>e.highlightField(t.field.id))})}function K(e,t){e.searchModal?.hide?.(),t.external?window.open(t.url,"_blank"):window.location.href=t.url}function Q(e){const t=i=>{(i.type==="action_page"||i.type==="import_page"||i.type==="tool_page")&&Z(e,i)};e.sections.forEach(i=>{t(i),i.subsections?.forEach(t)}),e.allFields.forEach(i=>{if(i.type==="field"&&i.field.type==="action_button")if(i.field.toggle_config){const s=i.field.has_dummy_data||!1;e.actionStates[i.field.id]={has_dummy_data:s,isLoading:!1,message:"",progress:0,success:null},e.settings[i.field.id]=s}else e.actionStates[i.field.id]={isLoading:!1,message:"",progress:0,success:null}})}function Z(e,t){const i=n=>{Array.isArray(n)&&n.forEach(o=>{o&&(o.id&&e.settings[o.id]===void 0&&o.default!==void 0?e.settings[o.id]=o.default:o.id&&e.settings[o.id]===void 0&&(e.settings[o.id]=""),o.type==="group"&&o.fields&&i(o.fields))})};i(t.fields);let s={isLoading:!1,message:"",progress:0,success:null,exportedFiles:[]};t.type==="import_page"&&(s={...s,uploadedFilename:"",uploadProgress:0,processingProgress:0,status:"idle",summary:{}}),e.actionStates[t.id]=s}function X(e){return Object.values(e.actionStates).some(t=>t.isLoading)}async function Y(e){const t=e.activePageConfig;if(!t||!t.ajax_action){console.error("Action page configuration not found.");return}const i=e.actionStates[t.id];i.isLoading=!0,i.message="Starting...",i.progress=0,i.processingProgress=0,i.success=null,i.exportedFiles=[],t.type==="import_page"&&(i.status="processing");const s={};if(t.fields?.forEach(o=>{o.id&&(s[o.id]=e.settings[o.id])}),t.type==="import_page"){const o=e.actionStates[t.id];o?.uploadedFilename&&(s.import_filename=o.uploadedFilename)}const n=async o=>{try{const a={action:window.ayecodeSettingsFramework.tool_ajax_action,nonce:window.ayecodeSettingsFramework.tool_nonce,tool_action:t.ajax_action,step:o,input_data:JSON.stringify(s)},r=await fetch(window.ayecodeSettingsFramework.ajax_url,{method:"POST",headers:{"Content-Type":"application/x-www-form-urlencoded"},body:new URLSearchParams(a)});if(!r.ok)throw new Error(`Server responded with status: ${r.status}`);const c=await r.json();i.success=c.success,c.data?.message&&(i.message=c.data.message);const l=c.data?.progress||0;c.data?.summary&&(i.summary=c.data.summary),t.type==="import_page"?i.processingProgress=l:i.progress=l,c.success&&c.data?.file&&i.exportedFiles.push(c.data.file),c.success&&c.data?.next_step!==null&&l<100?setTimeout(()=>n(c.data.next_step),20):(i.isLoading=!1,t.type==="import_page"&&(i.status="complete"))}catch(a){i.success=!1,i.message="An error occurred. Please check the console and try again.",i.isLoading=!1,t.type==="import_page"&&(i.status="complete"),console.error("Page action failed:",a)}};n(0)}async function ee(e,t){const i=e.allFields.find(d=>d.type==="field"&&d.field.id===t);if(!i){console.error("Action button configuration not found for:",t);return}const s=i.field,n=e.actionStates[t];let o;if(s.toggle_config?o=n.has_dummy_data?s.toggle_config.remove.ajax_action:s.toggle_config.insert.ajax_action:o=s.ajax_action,!o){console.error("No ajax_action defined for:",t);return}n.isLoading=!0,n.message="Starting...",n.progress=0,n.success=null;const a={};let c=document.getElementById(t)?.closest?.(".card-body")||e.$refs["action_container_"+t]||null;c&&c.querySelectorAll("input, select, textarea").forEach(g=>{const m=g.getAttribute("data-id")||g.id;m&&(a[m]=g.type==="checkbox"?g.checked:g.value)});const l=async d=>{try{const g={action:window.ayecodeSettingsFramework.tool_ajax_action,nonce:window.ayecodeSettingsFramework.tool_nonce,tool_action:o,step:d,input_data:JSON.stringify(a)},m=await fetch(window.ayecodeSettingsFramework.ajax_url,{method:"POST",headers:{"Content-Type":"application/x-www-form-urlencoded"},body:new URLSearchParams(g)});if(!m.ok)throw new Error(`Server responded with an error: ${m.status}`);const h=await m.json();n.success=h.success,h.data?.message&&(n.message=h.data.message),h.data?.progress&&(n.progress=h.data.progress),h.success&&h.data?.next_step!==null&&h.data?.progress<100?setTimeout(()=>l(h.data.next_step),20):(n.isLoading=!1,h.success&&s.toggle_config&&(n.has_dummy_data=!n.has_dummy_data,e.settings[t]=n.has_dummy_data),n.success&&setTimeout(()=>{n.message="",n.success=null},8e3))}catch(g){n.success=!1,n.message="Something went wrong, please refresh and try again.",n.isLoading=!1,console.error("Action failed:",g)}};l(0)}function te(e,t,i,s){const n=e.actionStates[i],o=t.dataTransfer?t.dataTransfer.files[0]:t.target.files[0];if(!o)return;const r=e.findPageConfigById(i,e.sections)?.accept_file_type;if(r){const d=o.name.split(".").pop().toLowerCase(),m={csv:"text/csv",json:"application/json"}[r];if(d!==r||m&&o.type!==m){n.status="error",n.success=!1,n.message=`Invalid file type. Please upload a .${r} file.`,t.target&&(t.target.value=null);return}}t.target&&(t.target.value=null),n.status="uploading",n.isLoading=!0,n.message="",n.success=null,n.uploadProgress=0;const c=new FormData;c.append("action",window.ayecodeSettingsFramework.file_upload_ajax_action),c.append("nonce",window.ayecodeSettingsFramework.tool_nonce),c.append("import_file",o);const l=new XMLHttpRequest;l.open("POST",window.ayecodeSettingsFramework.ajax_url,!0),l.upload.onprogress=d=>{d.lengthComputable&&(n.uploadProgress=Math.round(d.loaded*100/d.total))},l.onload=()=>{if(n.isLoading=!1,l.status>=200&&l.status<300){const d=JSON.parse(l.responseText);d.success?(n.status="selected",n.uploadedFilename=d.data.filename,n.message=d.data.message,e.settings[s]=d.data.filename):(n.status="error",n.success=!1,n.message=d.data.message||"File upload failed.")}else n.status="error",n.success=!1,n.message=`Upload error: ${l.statusText}`},l.onerror=()=>{n.isLoading=!1,n.status="error",n.success=!1,n.message="A network error occurred during upload."},l.send(c)}async function ie(e,t,i){const s=e.actionStates[t];if(!s?.uploadedFilename)return;const n=s.uploadedFilename;s.status="idle",s.uploadedFilename="",s.message="",s.success=null,e.settings[i]="";try{await fetch(window.ayecodeSettingsFramework.ajax_url,{method:"POST",headers:{"Content-Type":"application/x-www-form-urlencoded"},body:new URLSearchParams({action:window.ayecodeSettingsFramework.file_delete_ajax_action,nonce:window.ayecodeSettingsFramework.tool_nonce,filename:n})})}catch(o){console.error("Error deleting temp file:",o)}}async function se(e,t){if(e.loadedContentCache[t])return;const i=e.sections.find(s=>s.id===t);if(i?.ajax_content){e.isContentLoading=!0;try{const n=await(await fetch(window.ayecodeSettingsFramework.ajax_url,{method:"POST",headers:{"Content-Type":"application/x-www-form-urlencoded"},body:new URLSearchParams({action:window.ayecodeSettingsFramework.content_pane_ajax_action,nonce:window.ayecodeSettingsFramework.tool_nonce,content_action:i.ajax_content})})).json();e.loadedContentCache[t]=n.success?n.data.html:`<div class="alert alert-danger">Error: ${n.data?.message||"Could not load content."}</div>`}catch{e.loadedContentCache[t]='<div class="alert alert-danger">Request failed while loading content.</div>'}finally{e.isContentLoading=!1}}}function ne(e,t,i="info"){window.wp?.data?.dispatch("core/notices")?window.wp.data.dispatch("core/notices").createNotice(i==="error"?"error":"success",t,{type:"snackbar",isDismissible:!0}):window.aui_toast?.("asf-settings-framework-"+i,i,t)}function oe(e){const t=localStorage.getItem("asf_theme"),i=window.matchMedia&&window.matchMedia("(prefers-color-scheme: dark)").matches;e.theme=t||(i?"dark":"light"),e.$watch?.("theme",s=>localStorage.setItem("asf_theme",s))}function ae(e){e.theme=e.theme==="light"?"dark":"light"}function F(e){e.$nextTick(()=>{console.log("Re-initializing..."),typeof window.aui_init=="function"&&window.aui_init(),ue(e)})}function re(e,t){e.isChangingView||(e.isChangingView=!0,setTimeout(()=>{t(),e.$nextTick(()=>{e.isChangingView=!1,F(e)})},150))}function ce(e){window.addEventListener("beforeunload",t=>{(e.hasUnsavedChanges||e.isActionRunning)&&(t.preventDefault(),t.returnValue="A task is running or you have unsaved changes. Are you sure you want to leave?")}),document.addEventListener("keydown",t=>{(t.ctrlKey||t.metaKey)&&t.key==="k"&&(t.preventDefault(),e.searchModal?.show?.())}),window.addEventListener("hashchange",()=>e.handleUrlHash())}window.activeChoicesInstances=window.activeChoicesInstances||{},window.activeChoicesWatchers=window.activeChoicesWatchers||{};function le(e,t){setTimeout(()=>{const i=e.$refs[t];if(!i||!i.classList.contains("aui-select2"))return;window.activeChoicesWatchers[t]&&window.activeChoicesWatchers[t](),window.activeChoicesInstances[t]&&window.activeChoicesInstances[t].destroy();const s=e.editingField&&e.editingField._uid?"editingField":"settings",n=e[s],o=window.aui_get_choices_config?.(i),a=new window.Choices(i,o);window.activeChoicesInstances[t]=a,a.setChoiceByValue(String(n[t])),i.addEventListener("change",()=>{n[t]=a.getValue(!0)});const r=e.$watch(`${s}['${t}']`,c=>{if(!a.initialised)return;const l=a.getValue(!0);c!==l&&a.setChoiceByValue(String(c))});window.activeChoicesWatchers[t]=r},0)}function de(e,t){setTimeout(()=>{const i=e.$refs[t];if(!i)return;window.activeChoicesWatchers[t]&&window.activeChoicesWatchers[t](),window.activeChoicesInstances[t]&&window.activeChoicesInstances[t].destroy();const s=e.editingField&&e.editingField._uid?"editingField":"settings",n=e[s];Array.isArray(n[t])||(n[t]=[]);const o=window.aui_get_choices_config?.(i),a=new window.Choices(i,o);window.activeChoicesInstances[t]=a,a.setChoiceByValue(n[t]),i.addEventListener("change",()=>{const c=a.getValue(!0),l=n[t];JSON.stringify(l)!==JSON.stringify(c)&&(l.length=0,c.forEach(d=>l.push(d)))});const r=e.$watch(`${s}['${t}']`,c=>{if(!a.initialised)return;const l=a.getValue(!0);JSON.stringify(c)!==JSON.stringify(l)&&a.setChoiceByValue(c)});window.activeChoicesWatchers[t]=r},0)}function ue(e){document.querySelectorAll('input[data-aui-init="iconpicker"]').forEach(i=>{const s=()=>{const a=i.id;if(!a)return;const r=i.value;e.editingField&&e.editingField._uid&&Object.prototype.hasOwnProperty.call(e.editingField,a)?e.editingField[a]!==r&&(e.editingField[a]=r):e.settings[a]!==r&&(e.settings[a]=r)},n=()=>{i.dispatchEvent(new Event("change",{bubbles:!0})),s()};i.addEventListener("input",s),i.addEventListener("change",s),i.addEventListener("iconpickerSelected",n),i.addEventListener("iconpickerChange",n),i.addEventListener("change.bs.iconpicker",n),i.addEventListener("iconpicker-selected",n);const o=i.closest(".input-group")?.querySelector(".input-group-addon, .input-group-text");o&&o.addEventListener("click",()=>{setTimeout(n,0)})})}function ge(e,t){if(typeof window.wp>"u"||typeof window.wp.media>"u"){alert("WordPress media library not available.");return}const i=window.wp.media({title:"Select or Upload an Image",button:{text:"Use this image"},multiple:!1});i.on("select",()=>{const s=i.state().get("selection").first().toJSON();e.settings[t]=s.id;const n=s.sizes?.thumbnail?.url||s.sizes?.medium?.url||s.url;e.imagePreviews[t]=n}),i.open()}function pe(e,t){e.settings[t]=null,delete e.imagePreviews[t]}function me(e,t,i,s){if(typeof window.GeoDirectoryMapManager>"u"||typeof window.geodirMapData>"u"){console.error(`Cannot initialize GD Map for '${t}': GeoDirectory map scripts are not loaded on this page.`);const n=e.$refs[t+"_map_canvas"];n&&(n.innerHTML='<div class="alert alert-danger m-3">Error: GeoDirectory map scripts are not available.</div>');return}e.$nextTick(()=>{const n=e.$refs[t+"_map_canvas"];if(!n){console.error(`Map container not found for field '${t}'.`);return}const o=JSON.parse(JSON.stringify(window.geodirMapData));o.lat=e.settings[i]||o.default_lat,o.lng=e.settings[s]||o.default_lng,o.lat_lng_blank=!e.settings[i]&&!e.settings[s],o.prefix=`${t}_`;const a={onMarkerUpdate:r=>{e.settings[i]=parseFloat(r.lat).toFixed(6),e.settings[s]=parseFloat(r.lng).toFixed(6)}};window.GeoDirectoryMapManager.initMap(n.id,o,a)})}function fe(e,t){if(!t.show_if)return!0;try{return P(e,t.show_if)}catch(i){return console.error(`Error evaluating show_if for "${t.id}":`,i),!0}}function P(e,t){const s=t.replace(/\[%(\w+)%\]/g,(n,o)=>{const a=e[o];return typeof a=="string"?`'${a.replace(/'/g,"\\'")}'`:typeof a=="boolean"||typeof a=="number"?a:"null"}).split("||");for(const n of s){const o=n.split("&&");let a=!0;for(const r of o)if(!A(r.trim())){a=!1;break}if(a)return!0}return!1}function A(e){if(!["==","!=",">","<",">=","<="].some(c=>e.includes(c))){let c;try{c=JSON.parse(e.toLowerCase())}catch{c=e.trim()!==""}return!!c}const t=e.match(/^(.*?)\s*(==|!=|>|<|>=|<=)\s*(.*)$/);if(!t)throw new Error(`Invalid comparison: "${e}"`);let[,i,s,n]=t;const o=c=>(c=c.trim(),c.startsWith("'")&&c.endsWith("'")||c.startsWith('"')&&c.endsWith('"')?c.slice(1,-1):!isNaN(c)&&c!==""?parseFloat(c):c==="true"?!0:c==="false"?!1:c==="null"?null:c),a=o(i),r=o(n);switch(s){case"==":return a==r;case"!=":return a!=r;case">":return a>r;case"<":return a<r;case">=":return a>=r;case"<=":return a<=r;default:throw new Error("op")}}function he(e,t){e.$nextTick(()=>{const i=document.getElementById(t);if(!i)return;const s=i.closest(".row, .py-4, .border-bottom");s&&(s.scrollIntoView({behavior:"smooth",block:"center"}),s.classList.add("highlight-setting"),setTimeout(()=>s.classList.remove("highlight-setting"),3500))})}function E(e,t){for(const i of t){if(i.id===e)return i;if(i.subsections){const s=E(e,i.subsections);if(s)return s}}return null}window.__ASF_NULL_FIELD=new Proxy({},{get:(e,t)=>t==="hasOwnProperty"?i=>Object.prototype.hasOwnProperty.call(e,i):"",has:()=>!0});function we(){return{config:window.ayecodeSettingsFramework?.config||{},originalSettings:{},settings:{},strings:window.ayecodeSettingsFramework?.strings||{},imagePreviews:{},originalImagePreviews:{},currentSection:"",currentSubsection:"",searchQuery:"",isLoading:!1,sidebarOpen:!1,theme:"light",isChangingView:!1,searchModalEl:null,searchModal:null,allFields:[],customSearchLinks:[],sections:[],actionStates:{},isContentLoading:!1,loadedContentCache:{},accordionStates:{},leftColumnView:"field_list",editingField:null,sortIteration:0,activeSyncListeners:[],initialTargetValues:{},isValidating:!1,lastEditFieldCall:0,init(){oe(this),this.editingField=window.__ASF_NULL_FIELD,this.customSearchLinks=window.ayecodeSettingsFramework?.custom_search_links||[],T(this),this.settings=window.ayecodeSettingsFramework?.settings||{},this.imagePreviews=window.ayecodeSettingsFramework?.image_previews||{},j(this),this.sections.forEach(e=>{if(e.type==="form_builder"){Array.isArray(this.settings[e.id])||(this.settings[e.id]=[]);const t=e.templates.flatMap(i=>i.options);this.settings[e.id].forEach(i=>{const s=t.find(n=>n.id===i.template_id);s&&(i.fields=s.fields,i._template_icon=s.icon,s.fields.forEach(n=>{i[n.id]===void 0&&n.default!==void 0&&(i[n.id]=n.default),n.type==="toggle"&&i[n.id]===!0&&(i[n.id]=1)}))})}}),this.originalSettings=JSON.parse(JSON.stringify(this.settings)),this.originalImagePreviews=JSON.parse(JSON.stringify(this.imagePreviews)),Q(this),L(this),W(this),ce(this),F(this),this.$watch("leftColumnView",(e,t)=>{e==="field_list"&&t==="field_settings"&&this.clearSyncListeners()})},get activePageConfig(){return U(this)},get hasUnsavedChanges(){return I(this)},get currentSectionData(){return y(this)},get currentSubsectionData(){return x(this)},get isSettingsPage(){return k(this)},get isActionRunning(){return X(this)},get groupedSearchResults(){return G(this)},get duplicateKeys(){const e=this.activePageConfig?.unique_key_property;if(!e)return[];const i=(this.settings[this.activePageConfig.id]||[]).reduce((s,n)=>{const o=n[e];return o&&(s[o]=(s[o]||0)+1),s},{});return Object.keys(i).filter(s=>i[s]>1)},get parentFields(){return(this.settings[this.activePageConfig?.id]||[]).filter(t=>!t._parent_id||t._parent_id==0)},childFields(e){return(this.settings[this.activePageConfig?.id]||[]).filter(i=>i._parent_id==e)},get otherFields(){return!this.activePageConfig||this.activePageConfig.type!=="form_builder"||!this.editingField?._uid?[]:(this.settings[this.activePageConfig.id]||[]).filter(t=>t._uid!==this.editingField._uid).map(t=>({label:t.label,value:t.key||t.htmlvar_name||t._uid,_uid:t._uid}))},toggleTheme(){ae(this)},reinitializePlugins(){F(this)},changeView(e){re(this,e)},goToSearchResult(e){z(this,e)},goToSection(e,t=""){this.activePageConfig?.type==="form_builder"&&(this.editingField=window.__ASF_NULL_FIELD,this.leftColumnView="field_list"),H(this,e,t)},goToCustomLink(e){K(this,e)},switchSection(e){this.activePageConfig?.type==="form_builder"&&(this.editingField=window.__ASF_NULL_FIELD,this.leftColumnView="field_list"),q(this,e)},switchSubsection(e){B(this,e)},highlightField(e){he(this,e)},handleUrlHash(){L(this)},updateUrlHash(e=null){b(this,e)},setInitialSection(){_(this)},async saveSettings(){await V(this)},discardChanges(){M(this)},shouldShowField(e){const t=this.editingField&&this.editingField._uid?this.editingField:this.settings;return fe(t,e)},evaluateCondition(e){return P(this,e)},evaluateSimpleComparison(e){return A(e)},renderField(e,t="settings",i=null){return!e||typeof e!="object"||!e.type?"":(i||this.activePageConfig,R(e,t))},selectImage(e){ge(this,e)},removeImage(e){pe(this,e)},initGdMap(e,t,i){me(this,e,t,i)},initChoice(e){le(this,e)},initChoices(e){de(this,e)},async executePageAction(){await Y(this)},async executeAction(e){await ee(this,e)},handleFileUpload(e,t,i){te(this,e,t,i)},async removeUploadedFile(e,t){await ie(this,e,t)},async loadCustomPageContent(e){await se(this,e)},async saveForm(){if(this.leftColumnView==="field_settings"&&!this.validateEditingField())return;const e=this.activePageConfig.id;(this.settings[e]||[]).forEach(i=>{const s=i._parent_id===null||i._parent_id===void 0?0:i._parent_id;i._parent_id=s,"tab_parent"in i&&(i.tab_parent=s),"tab_level"in i&&(i.tab_level=s?1:0)}),await J(this)},countFieldsByTemplateId(e){const t=this.settings[this.activePageConfig.id]||[],i=e.defaults&&e.defaults.field_type_key?e.defaults.field_type_key:e.base_id||e.id;return t.filter(s=>(s.field_type_key||s.template_id)===i).length},handleFieldClick(e){if(e.limit&&this.countFieldsByTemplateId(e)>=e.limit){window.aui_toast?.("asf-limit-reached","error","This field is single use only and is already being used.");return}this.addField(e)},addField(e){let t=e,i=null;if(e.base_id){if(t=this.activePageConfig.templates.flatMap(r=>r.options).find(r=>r.id===e.base_id),!t){alert(`Error: Base template with id '${e.base_id}' could not be found.`);return}i=e.defaults||{}}const s=a=>a.reduce((r,c)=>(c.id&&(r[c.id]=c.default!==void 0?c.default:null),c.type==="group"&&c.fields&&Object.assign(r,s(c.fields)),c.type==="accordion"&&c.fields&&c.fields.forEach(l=>{l.fields&&Object.assign(r,s(l.fields))}),r),{}),n=s(t.fields);if(n._uid="new_"+Date.now(),n.is_new=!0,n.template_id=t.id,n.fields=t.fields,n._template_icon=t.icon,n._parent_id=0,"tab_parent"in n&&(n.tab_parent=0),"tab_level"in n&&(n.tab_level=0),i)for(const a in i)Object.prototype.hasOwnProperty.call(n,a)&&(n[a]=i[a]);const o=this.activePageConfig?.unique_key_property;if(o&&n[o]){const a=this.settings[this.activePageConfig.id]||[];let r=n[o],c=2;for(;a.some(l=>l[o]===r);)r=`${n[o]}${c}`,c++;n[o]=r}this.settings[this.activePageConfig.id].push(n),this.editField(n)},slugify(e){return String(e).normalize("NFKD").replace(/[\u0300-\u036f]/g,"").trim().toLowerCase().replace(/[^a-z0-9 -]/g,"").replace(/\s+/g,"_").replace(/-+/g,"_")},findSchemaById(e,t){for(const i of e){if(i.id===t)return i;if(i.fields){const s=this.findSchemaById(i.fields,t);if(s)return s}}return null},validateEditingField(){if(this.isValidating)return!0;this.isValidating=!0;try{if(!this.editingField||!this.editingField.fields)return!0;const e=document.getElementById("asf-field-settings");e&&e.querySelectorAll(".asf-field-error").forEach(i=>i.classList.remove("asf-field-error"));const t=i=>{for(const s of i){if(s.extra_attributes?.required){const n=this.editingField[s.id];if(n===""||n===null||n===void 0)return this.showNotification(`Error: The "${s.label||s.id}" field is required.`,"error"),this.$nextTick(()=>{const o=document.getElementById(s.id);if(o){const a=o.closest(".accordion-collapse");a&&!a.classList.contains("show")&&new bootstrap.Collapse(a).show();const r=o.closest(".row");r&&(r.classList.add("asf-field-error"),r.scrollIntoView({behavior:"smooth",block:"center"}),setTimeout(()=>{r.classList.remove("asf-field-error")},3500))}}),!1}if(s.fields&&Array.isArray(s.fields)&&!t(s.fields))return!1}return!0};return t(this.editingField.fields)}finally{this.isValidating=!1}},editField(e){const t=performance.now();if(t-this.lastEditFieldCall<100||(this.lastEditFieldCall=t,this.editingField?._uid===e._uid))return;const i=e;if(this.editingField&&this.editingField._uid&&this.editingField._uid!==e._uid){if(this.clearSyncListeners(),!this.validateEditingField()){this.$nextTick(()=>{this.setupWatchersForField(this.editingField)});return}}else this.clearSyncListeners();this.editingField=window.__ASF_NULL_FIELD,this.leftColumnView="field_list",this.$nextTick(()=>{const s=this.getTemplateForField(i);s&&(i.fields=s.fields),document.querySelector(".tooltip")?.remove(),this.initialTargetValues={},this.editingField=i,this.leftColumnView="field_settings",this.$nextTick(()=>{this.setupWatchersForField(i),this.reinitializePlugins()})})},setupWatchersForField(e){if(!e||!e.fields)return;const t=i=>{Array.isArray(i)&&i.forEach(s=>{if(s.syncs_with&&Array.isArray(s.syncs_with)){s.syncs_with.forEach(o=>{e[o.target]!==void 0&&(this.initialTargetValues[o.target]=e[o.target])});const n=this.$watch(`editingField.${s.id}`,o=>{this.editingField._uid===e._uid&&this.handleSync(s.id,o)});this.activeSyncListeners.push(n)}s.fields&&t(s.fields)})};t(e.fields)},handleSync(e,t){if(!this.editingField||!this.editingField._uid||!this.editingField.fields)return;const i=this.getTemplateForField(this.editingField);if(!i)return;const s=this.findSchemaById(i.fields,e);!s||!s.syncs_with||s.syncs_with.forEach(n=>{const o=n.target;if(this.editingField[o]!==this.initialTargetValues[o])return;const a=n.transform==="slugify"?this.slugify(t):t;this.editingField[o]=a,this.initialTargetValues[o]=a})},clearSyncListeners(){for(;this.activeSyncListeners.length>0;){const e=this.activeSyncListeners.pop();if(typeof e=="function")try{e()}catch(t){console.error("Error clearing watcher:",t)}}this.activeSyncListeners=[]},closeEditingField(){this.validateEditingField()&&(this.clearSyncListeners(),this.leftColumnView="field_list",this.$nextTick(()=>{this.editingField=window.__ASF_NULL_FIELD}))},deleteField(e){if(e._is_default){alert("This is a default field and cannot be deleted.");return}if(!confirm("Are you sure you want to delete this field?"))return;let t=this.settings[this.activePageConfig.id];const i=t.findIndex(s=>s._uid===e._uid);i>-1&&t.splice(i,1),this.settings[this.activePageConfig.id]=t.filter(s=>s._parent_id!==e._uid),this.editingField&&this.editingField._uid===e._uid&&(this.editingField=window.__ASF_NULL_FIELD,this.leftColumnView="field_list")},handleSort(e,t,i=null){const s=this.activePageConfig.id;let n=[...this.settings[s]];const o=n.find(d=>d._uid==e);if(!o)return;if(i){const d=n.find(h=>h._uid==i),g=this.getTemplateForField(d),m=this.getTemplateForField(o);if(g&&g.allowed_children){if(g.allowed_children[0]!=="*"&&(!m||!g.allowed_children.includes(m.id))){alert(`A "${m?.title}" field cannot be placed inside a "${g.title}".`),this.sortIteration++;return}}else if(!this.activePageConfig.nestable){alert("Nesting is not enabled for this field."),this.sortIteration++;return}}if(i!==null&&n.some(d=>d._parent_id===o._uid)){alert("Items that already have children cannot be nested."),this.sortIteration++;return}const a=i===null?0:i;o._parent_id=a,"tab_parent"in o&&(o.tab_parent=a),"tab_level"in o&&(o.tab_level=a?1:0);const r=n.indexOf(o);n.splice(r,1);const c=n.filter(d=>(d._parent_id===null?0:d._parent_id)==a);let l;if(t>=c.length){const d=c.length>0?c[c.length-1]:null;if(d){const g=n.indexOf(d),m=n.findLastIndex?n.findLastIndex(h=>h._parent_id===d._uid):-1;l=m!==-1?m+1:g+1}else a?l=n.findIndex(g=>g._uid===a)+1:l=n.length}else{const d=c[t];l=n.indexOf(d)}n.splice(l,0,o),this.settings[s]=n,this.sortIteration++},addCondition(){this.editingField.conditions||(this.editingField.conditions=[]),this.editingField.conditions.push({action:"",field:"",condition:"",value:""})},removeCondition(e){this.editingField.conditions.splice(e,1)},getTemplateForField(e){if(!e||!e.template_id)return null;const t=this.activePageConfig;return t&&t.templates?t.templates.flatMap(s=>s.options).find(s=>s.id===e.template_id):null},findPageConfigById(e,t){return E(e,t)},showNotification(e,t){ne(this,e,t)}}}const S=new Map;let $=typeof window<"u"?window.asfFieldRenderer:void 0;function u(e,t){S.set(e,t)}function be(e,t="settings"){if(!e||!e.type)return'<div class="alert alert-warning">Invalid field configuration</div>';const i=S.get(e.type);if(typeof i=="function")try{return i(e,t)}catch(n){return console.error(`Renderer for type "${e.type}" threw:`,n),`<div class="alert alert-danger">Error rendering field type: ${e.type}</div>`}const s=window.__asfFieldRendererLegacy||$;return s&&typeof s.renderField=="function"?s.renderField(e,t):`<div class="alert alert-info">Unsupported field type: ${e.type}</div>`}const O=e=>"render"+e.charAt(0).toUpperCase()+e.slice(1)+"Field";function p(e,t,i="settings"){const s=S.get(e);if(typeof s=="function")try{return s(t,i)}catch(o){console.error(`Renderer "${e}" error:`,o)}const n=window.__asfFieldRendererLegacy||$;return n&&typeof n[O(e)]=="function"?n[O(e)](t,i):n&&typeof n.renderField=="function"?n.renderField(t,i):`<div class="alert alert-info">Unsupported field type: ${e}</div>`}const ve=(e,t)=>p("text",e,t),ye=(e,t)=>p("email",e,t),$e=(e,t)=>p("url",e,t),_e=(e,t)=>p("alert",e,t),Fe=(e,t)=>p("password",e,t),Se=(e,t)=>p("google_api_key",e,t),Ce=(e,t)=>p("number",e,t),ke=(e,t)=>p("textarea",e,t),xe=(e,t)=>p("toggle",e,t),Le=(e,t)=>p("select",e,t),Pe=(e,t)=>p("color",e,t),Ae=(e,t)=>p("range",e,t),Ee=(e,t)=>p("checkbox",e,t),Oe=(e,t)=>p("radio",e,t),Ne=(e,t)=>p("multiselect",e,t),Te=(e,t)=>p("checkbox_group",e,t),je=(e,t)=>p("group",e,t),Ie=(e,t)=>p("image",e,t),De=(e,t)=>p("hidden",e,t),Ve=(e,t)=>p("file",e,t),Je=(e,t)=>p("font-awesome",e,t),Me=(e,t)=>p("gd_map",e,t),Re=(e,t)=>p("helper_tags",e,t),Ue=(e,t)=>p("action_button",e,t),He=(e,t)=>p("link_button",e,t),qe=(e,t)=>p("custom_renderer",e,t);(function(){typeof window>"u"||($&&(window.__asfFieldRendererLegacy=$),window.asfFieldRenderer={renderField:be,renderTextField:ve,renderEmailField:ye,renderUrlField:$e,renderAlertField:_e,renderPasswordField:Fe,renderGoogleApiKeyField:Se,renderNumberField:Ce,renderTextareaField:ke,renderToggleField:xe,renderSelectField:Le,renderColorField:Pe,renderRangeField:Ae,renderCheckboxField:Ee,renderRadioField:Oe,renderMultiselectField:Ne,renderCheckboxGroupField:Te,renderGroupField:je,renderImageField:Ie,renderHiddenField:De,renderFileField:Ve,renderIconField:Je,renderGdMapField:Me,renderHelperTagsField:Re,renderActionButtonField:Ue,renderLinkButtonField:He,renderCustomField:qe,__registerRenderer:u})})();function Be(e){return String(e).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function N(e){return String(e).replace(/"/g,"&quot;")}function w(e){if(!e?.extra_attributes||typeof e.extra_attributes!="object")return"";const t=[];for(const[i,s]of Object.entries(e.extra_attributes)){const n=i.replace(/[^a-zA-Z0-9-]/g,"");n&&(s===!0?t.push(n):t.push(`${n}="${Be(s)}"`))}return t.join(" ")}u("hidden",e=>{const t=w(e);return`<input type="hidden" id="${e.id}" name="${e.id}" x-model="settings.${e.id}" ${t}>`}),u("alert",e=>`
    <div class="alert alert-${e.alert_type||"info"} mb-0">
      ${e.label?`<h6 class="alert-heading">${e.label}</h6>`:""}
      ${e.description||""}
    </div>
  `);function f(e){return e?.custom_desc?`<div class="form-text mt-2">${e.custom_desc}</div>`:""}u("text",C),u("email",C),u("url",C);function C(e){const t=e.class||"",i=w(e),s=N(e.placeholder||"");let n="";if(e.active_placeholder&&e.placeholder){const l=JSON.stringify(e.placeholder);n=`
      @focus='if (!settings.${e.id}) { settings.${e.id} = ${l}; }'
      @blur='if (settings.${e.id} === ${l}) { settings.${e.id} = ""; }'
    `}const a=`<input type="${e.type||"text"}" class="form-control ${t}" id="${e.id}" name="${e.id}" x-model="settings.${e.id}" placeholder="${s}" ${i} ${n}>`,r=e.input_group_right?`<div class="input-group">${a}${e.input_group_right}</div>`:a,c=f(e);return`
  ...
  `}),u("textarea",e=>{const t=e.rows||5,i=e.class||"",s=w(e),n=N(e.placeholder||""),o=f(e);let a="";if(e.active_placeholder&&e.placeholder){const r=JSON.stringify(e.placeholder);a=`
  ...
  `}),typeof window<"u"&&(window.ayecodeSettingsApp=we),document.addEventListener("DOMContentLoaded",function(){if(typeof window.Alpine>"u"){console.error("Alpine.js is required for AyeCode Settings Framework");return}console.log("AyeCode Settings Framework ready")}),document.addEventListener("alpine:init",()=>{Alpine.directive("sort")?console.log("x-sort directive is available ✅"):console.log("x-sort directive not found ❌")})})();
