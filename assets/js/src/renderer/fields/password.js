...
registerRenderer('password', (field) => {
    const customClass = field.class || '';
    const extra = renderExtraAttributes(field);
    const customDescHtml = renderCustomDesc(field);
    const input = `<input type="password" autocomplete="new-password" class="form-control ${customClass}" id="${field.id}" name="${field.id}" x-model="settings.${field.id}" placeholder="${field.placeholder || ''}" ${extra}>`;
